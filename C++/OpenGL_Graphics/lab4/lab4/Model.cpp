...
Model::Model(){
	...
	textureInit("cylinder2.bmp", _textures[0]);
	textureInit("teapot.bmp", _textures[1]);
	textureInit("floor.bmp", _textures[2]);

	_sphere = new Sphere(genListId(), _textures[2]);
	_conus = new ConusAnimator(genListId(), _textures[2]);
	_plane = new Plane(genListId(), _textures[0]);
}
