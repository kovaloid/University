...
void Parser::statement()
{
	...
	// или еще одно присваивание..
	// Записываем значение выражения по адресу наших переменных
	if(see(T_IDENTIFIER)) {
		vector<int> varAddr(0); // вектор, для хранения адресов переменных
		varAddr.push_back(findOrAddVariable(scanner_->getStringValue())); //записываем адрес первой переменной
		next(); 
		mustBe(T_ASSIGN); //далее обязательно должно быть присвоение
		factorExist = false; 
		do{
			if (see(T_IDENTIFIER)){ //смотрим дальше. если снова встретили переменную
				int varAddress = findOrAddVariable(scanner_->getStringValue()); // находим или создаем ее адрес
				next();
				if (see(T_ASSIGN)){ // если снова встретили присвоение то кладем адрес в массив
					varAddr.push_back(varAddress);
					next();
				}
				else{//если после переменной мы встретили не присвоение а что-либо другое. значит уже началось выражение.
					//выгружаем эту переменную и выходим из цикла
		...
		expression();
		factorExist = false;
		//достаем последний сохраненный адрес
		int addr_last = varAddr.at(varAddr.size()-1);		
		codegen_->emit(STORE, addr_last); // сохраняем переменную по этому адресу
		varAddr.pop_back(); //выгружаем адрес
		while (varAddr.size()>0){ // пока есть еще адреса 
			int addr = varAddr.at(varAddr.size()-1); // достаем следующий адрес
			codegen_->emit(LOAD, addr_last); // выгружаем значение по адресу последней переменной
			codegen_->emit(STORE, addr); //сохраняем значение по адресу текущей переменной
			varAddr.pop_back(); //выгружаем адрес
		}
	}
...
void Parser::expression()
{
	...
	while(see(T_ADDOP)) {
		...
		next();
		factorExist = false; //NNEEEEEEEWWWW!!!
		term();
...
void Parser::term()
{
	...
	*/
	if (!factorExist) factor(); //NEEEEEEEWWWW changed from simple - factor();
	while(see(T_MULOP)) {
		Arithmetic op = scanner_->getArithmeticValue();
		next();
		factor();
